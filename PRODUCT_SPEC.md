...
## 10. Milestones
- M0: daily capture + indexing + narrow-to-new-node
- M1: stable node identity + basic traversal + side window skeleton
- M2: transclusion + backlinks (node+subtree) + slash commands
- M3: citations (schemes) + sources(YAML) → BibTeX generation + `gd` UX
- M4: views/query DSL + embeddable views + project export scaffolding
- M5: creative traversal affordances + accusation prompts
